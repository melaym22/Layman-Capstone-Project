...
CAPDAP Final Project 12/19/2024
...
o_sma1 <-ggplot(WC, aes(x =DateTime))+
  geom_line(aes(y = DO), color = "blue", size = 0.3) + #original time series
  geom_line(aes(y = sma1), color = "red", size = 0.2) + #moving average
...
Here, I'm going to make a new variable called DateTime, which will combine date and time to ensure a 24 hour timeline over
...
```{r}
ggplot(WC,aes(x=DateTime,y=DO))+
  geom_point(aes(fill=Site_Type), colour="black", shape=21,fill=WC$Site_Type, size=1.5)+
