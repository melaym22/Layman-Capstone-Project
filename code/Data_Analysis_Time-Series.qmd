...
```{r}
hurricane <- data.frame(X=c(WC$DateTime[37],WC$DateTime[225]), Y = 100)
...
o_sma1 <-ggplot(WC, aes(x =DateTime))+
  ...
  theme_bw()+
  geom_label(aes(x = WC$DateTime[115], y = 10, label = "Vertical Bars = Night"), fill = "lightblue")+
  geom_label(aes(x = WC$DateTime[57], y = 96, label = "Active Hurricane Beryl"), fill = "red")
o_sma1
```
